...
function bookInfo(isbn){
	$.ajax({
		...
		success: function(data){
			...
			temp += "<tr><td class=g>출 판 사</td><td><input type='text' name='publisher' value='"+data.publisher+"'></td></tr>";
...
function bookInsert(){//하나의 Book객체를 서버로 전송하는 기능.. 처음나왔다. 비동기로 보낸다(JSON 타입으로 바꿔 전송).
	$.ajax({
		url: "api/book",
		type: "POST",
		data: JSON.stringify({
